...
#curPosPan = round((_PanServoUL + _PanServoLL)/2)
...
fps = FPS().start() #See if you can get a FPS counter going
vs = WebcamVideoStream(src=0).start() #If src is changed to a URL it will use that as the camera

#while fps._numFrames < 6000: #runs for 6000 frames
while True:
	...
	if faceFound == True:
		...
		if lastface == 1:
			...
			cv2.putText(gray, "Left Profile Detected", (x,y-10), cv2.FONT_HERSHEY_SIMPLEX, .5, (255,255,255), 1, cv2.LINE_AA)
			cv2.circle(gray, (int(Cface[0]), int(Cface[1])), 1, 255, 2)
			cv2.putText(gray, "Center", (int(Cface[0]), int(Cface[1])), cv2.FONT_HERSHEY_SIMPLEX, .3, (255,255,255), 1, cv2.LINE_AA)
		...
		if cv2.waitKey(1) & 0xFF == ord('q'):
			break
		boundFace = 10
		distance = 1 #speed modifier for distance
		...
		if int(Cface[0]) > (camCenter[0]) and absoluteDistanceX > boundFace:
			print(str(PanServo) + '=' + str(curPosPan) + '\n')
			PanMove(distance,1,curPosPan, desPosPan)			
			centerPan = "right "
		elif int(Cface[0]) < (camCenter[0]) and absoluteDistanceX < boundFace:
			centerPan = "left "
			PanMove((distance*-1),1,curPosPan, desPosPan)
			print(str(PanServo) + '=' + str(curPosPan) + '\n')
		
		if int(Cface[1]) > (camCenter[1]) and absoluteDistanceY > boundFace:
			print(str(PanServo) + '=' + str(curPosPan) + '\n')
			TiltMove(distance,1,curPosTilt, desPosTilt)
			centerTilt = "lower "		
		elif int(Cface[1]) < (camCenter[1]) and absoluteDistanceY < boundFace:
			print(str(PanServo) + '=' + str(curPosPan) + '\n')
			TiltMove((distance*-1),1,curPosTilt, desPosTilt)
			centerTilt = "upper "
		
		if Cface[0] == camCenter[0] and Cface[1] == camCenter[1]:
			break
	...
	if not faceFound:		# if no face was found...-
		...
		WhereTo = bool(random.getrandbits(1)) #boolean for either random up or down movement
		if WhereTo == True:
